...
# --- 🔧 CONFIGURATION ---
TELEGRAM_BOT_TOKEN = "8542050953:AAHx-mt8RqhvkfiFKoUT9M_3QZl0y1A_PBE"
TELEGRAM_CHAT_ID = "1119772887"
...
<head>
    ...
    <style>
        ...
        .btn-telegram { background: #2980b9; width: 100%; margin-top: 5px; }
        ...
        .actions-section { margin-top: 30px; display: flex; gap: 20px; flex-wrap: wrap; }
        .action-card { flex: 1; padding: 20px; background: #ecf0f1; border-radius: 8px; border: 1px solid #bdc3c7; min-width: 250px; }
    ...
    <div class="container">
        ...
        <div class="actions-section">
            ...
            <div class="action-card">
                ...
                <p>Send instant alert to configured chat.</p>
                <form method="POST" action="/send_telegram">
                    <button type="submit" class="btn btn-telegram">Send Telegram Update</button>
                </form>
            </div>
        </div>
        {% endif %}
...
def send_email(recipient, image_path, count, status):
    ...
    if count == 0:
        ...
        🔍 OBSERVATIONS:
        No visible pests were detected in the analyzed image. The foliage appears clean.

        🛡️ RECOMMENDATIONS:
        1. Monitor: Continue regular monitoring (suggested every 2-3 days).
        2. Maintenance: Ensure optimal watering and lighting conditions are met.
        3. Prevention: Keep the soil surface clean of dead leaves to discourage future pests.
        
        ---------------------------------
        Auto-generated by Raspberry Pi Smart Crop System
        """
    else:
        # Infected Report Body
        severity = "LOW" if count < 5 else "HIGH"
        body = f"""
        SMART CROP MONITORING REPORT
        =================================
        Date: {timestamp}
        ...
        🔍 OBSERVATIONS:
        Potential pest activity detected! The system identified {count} anomalies on the leaves.

        🚑 IMMEDIATE ACTIONS REQUIRED:
        1. ISOLATE: Separate this plant from others immediately to prevent spread.
        2. INSPECT: Manually check the underside of leaves and stems for bugs.
        3. TREAT: 
           - Wash leaves with a mild insecticidal soap or water solution.
           - Apply Neem Oil for organic control if aphids/mites are found.
...
def send_telegram():
    ...
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto"
        with open(current_analysis['path'], "rb") as img_file:
            payload = {
                "chat_id": TELEGRAM_CHAT_ID,
                "caption": f"⚠️ <b>MANUAL ALERT</b>\nPests: {current_analysis['count']}\nStatus: {current_analysis['status']}",
                "parse_mode": "HTML"
            }
            files = {"photo": img_file}
            ...
            if resp.status_code == 200:
                ...
                flash(f"❌ API Error: {resp.text}", "error")
    except Exception as e:
